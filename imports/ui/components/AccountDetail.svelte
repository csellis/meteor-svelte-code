                ...
                <dl class="sm:divide-y sm:divide-gray-200">
                  <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dd class="mt-1 text-2xl text-gray-900 sm:mt-0 sm:col-span-2 text-right">
                      {#if selectedAccount}
                        <MoneyString amount={selectedAccount.balances.current} denomination={selectedAccount.iso_currency_code} />
                      {/if}
                    </dd>
                  </div>
                  <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    ...
                    <dd on:click={handleEditName} class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 cursor-pointer">
                      {#if editName}

                        <div>
                          <label for="email" class="block text-sm font-medium text-gray-700 sr-only">Search candidates</label>
                          <div class="mt-1 flex rounded-md shadow-sm">
                            <div class="relative flex items-stretch flex-grow focus-within:z-10">
                              <input bind:value={selectedAccount.name} type="text" name="email" id="email" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full rounded-none rounded-l-md pl-10 sm:text-sm border-gray-300" placeholder="John Doe">
                            </div>
                            <button on:click={handleSubmit} class="-ml-px relative inline-flex items-center space-x-2 px-4 py-2 border border-gray-300 text-sm font-medium rounded-r-md text-gray-700 bg-gray-50 hover:bg-gray-100 focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
